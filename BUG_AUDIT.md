...
## 迭代记录（2026-08-08 续 14，会话日志驱动）
- WRITE-ERR-001（P3，工具报错可操作性）：write 对已存在文件不带 revision 时，already_exists 报错未附当前 revision，模型需再 read 一次才能重试（会话日志 seq34→35 实测：先 read 却漏传 revision，失败后需重试）。已修：报错附“当前 revision: b3:...”（与 edit stale_revision 对齐）。回归：p2_fixes.rs already_exists 断言扩展。