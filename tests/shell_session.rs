...
async fn cancel_keeps_last_confirmed_and_recovers() {
    ...
    let handle = {
        let ctx = ctx.clone();
        tokio::spawn(async move { run_bash(&ctx, "cd b; sleep 60", 120_000, cancel_inner).await })
    };
...
async fn exported_env_persists_across_calls() {
    ...
    let state = ctx.shell.lock().unwrap();
    assert_eq!(
        state.env_overlay.set.get("TPI_FOO").map(String::as_str),
        Some("abc")
    );
}
...
async fn cwd_matrix_full_sequence() {
    ...
    let r = run_bash(&ctx, "pwd", 60_000, cancel.clone()).await;
    assert!(
        stdout_part(&r.model_payload.output).contains("/src")
            || stdout_part(&r.model_payload.output).contains("\\src"),
