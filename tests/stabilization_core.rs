...
/// §21 场景 8：compaction 后 runtime/replay 等价（含摘要注入语义）。
#[tokio::test]
async fn replay_after_compaction_matches_runtime() {
    let dir = tempfile::tempdir().unwrap();
    let workspace = Utf8PathBuf::from_path_buf(dir.path().to_path_buf()).unwrap();
    let mut h = ReplayHarness::new(&workspace);
    ...
    // compaction——触发时 history 已有足够多消息可被显著压缩）；
    // compaction 请求（tools 空）返回结构化 summary。
    ...
    std::fs::write(workspace.join("probe.txt"), "probe-content ".repeat(150)).unwrap();
    ...
    let events = tpi::session::read_events(h.session.path()).expect("read events");
    let has_compaction = events
        .iter()
        .any(|e| matches!(e, tpi::session::SessionEvent::CompactionCommitted { .. }));
    assert!(has_compaction, "本场景必须触发 compaction");

    let resumed = h.replay();
    assert_eq!(
        outcome.messages, resumed,
        "compaction 后 runtime 与 replay 必须等价"
    );
    // 摘要必须注入且位于最前（§15.4：不伪装成新 user intent）。
    assert!(
        matches!(&resumed[0], ChatMessage::User(t) if t.contains("压缩摘要")),
        "最新 summary 必须作为前缀注入: {:?}",
        resumed[0]
    );
    // 消息单元原子性（§18.2）：resumed 中任何 Tool 消息之前必须有
    // Assistant(tool_calls) 载体（compaction 边界不得切开 tool protocol）。
    let mut saw_assistant_with_calls = false;
    for m in &resumed[1..] {
        match m {
            ChatMessage::Assistant { tool_calls, .. } => {
                saw_assistant_with_calls = !tool_calls.is_empty();
            }
            ChatMessage::Tool { .. } => {
                assert!(
                    saw_assistant_with_calls,
                    "Tool 消息前必须有 Assistant(tool_calls) 载体（§18.2）"
                );
                saw_assistant_with_calls = false;
            }
            _ => saw_assistant_with_calls = false,
        }
    }
}
